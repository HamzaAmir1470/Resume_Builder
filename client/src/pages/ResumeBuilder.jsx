...
import {
  ArrowLeftIcon, Briefcase, FileText, GraduationCap, User, FolderIcon,
  Sparkles, ChevronLeft, ChevronRight, Share2Icon, EyeIcon, EyeOffIcon, DownloadIcon
} from 'lucide-react';
...
const saveResume = async () => {
  try {
    ...
    // Append removeBackground if needed
    if (removeBackground) formData.append("removeBackground", "yes");

    // Append image ONLY if it's a File (not string URL)
        ...
        <div className="lg:col-span-7 max-lg:mt-6">
          <div className="relative w-full">
            <div className="absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2">
              ...
              <button onClick={downloadResume} disabled={isDownloading} aria-busy={isDownloading} className={`flex items-center px-6 py-2 gap-2 text-xs bg-gradient-to-br from-green-100 to-green-200 text-green-600 rounded-lg ring-green-300 hover:ring transition-colors ${isDownloading ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
                <DownloadIcon className="size-4" /> {isDownloading ? 'Preparing...' : 'Download'}
              </button>
            </div>
          </div>
